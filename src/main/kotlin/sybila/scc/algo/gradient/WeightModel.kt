...
fun main() {
    ...
    val maxX = 0.2
    val maxB = 1200.0
    val image = BufferedImage(1000, 2000, BufferedImage.TYPE_INT_ARGB)
    ...
    model.findComponents { component ->
        ...
        for (s in 0 until model.stateCount) {
            ...
            if (p != null) {
                val pixelYLow = Math.floor(2000 * model.queueInterval(s).low / maxB).toInt()
                val pixelYHigh = Math.ceil(2000 * model.queueInterval(s).high / maxB).toInt()
                for (pixelY in pixelYLow..pixelYHigh) {
                    p.drawRow(image = image, y = 2000 - pixelY, pixelsX = 1000, min = minX, max = maxX)
                }
