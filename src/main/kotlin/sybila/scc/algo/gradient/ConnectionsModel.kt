...
class ConnectionsModel(
    ...
    private fun nextPMax(avrQueue: Interval, pMax: Interval): Interval {
        val a = (currentQueueMax(dropRate(avrQueue, pMax)) - i(qTarget)) * i(gamma)
        ...
        return (pMax + a * doubleArrayOf(Math.sqrt(b.low), Math.sqrt(b.high))).boundDown(pMaxBounds.low).boundUp(pMaxBounds.high)
    }
...
fun main() {
    ...
    println("Write image...")
    ImageIO.write(image, "PNG", File("gred_connections.png"))
}